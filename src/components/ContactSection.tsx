...
const ContactSection = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setTimeout(() => {
      setLoading(false);
      toast.success("Formulário enviado! Entraremos em contato via WhatsApp em até 24h.");
      ...
      <div className="container max-w-3xl">
        ...
        >
          ...
          <p className="text-muted-foreground">
            Preencha o formulário e nossa equipe desenhará a estratégia de mentoria mais eficiente para o seu caso.
          </p>
        </motion.div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-3">
                {["Primeira Habilitação", "Aperfeiçoamento Urbano", "Especialização Rodoviária"].map(
                  (opt) => (
                    <label
                      key={opt}
                    ...
                    >
                      <input type="radio" name="objetivo" value={opt} required className="accent-electric" />
                      {opt}
                    </label>
                  )
                )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-3">
                {[1, 2, 3, 4, 5].map((n) => (
                  <label
                    key={n}
                    className="flex items-center justify-center w-12 h-12 rounded-2xl border border-border hover:border-electric cursor-pointer transition-colors font-heading font-bold"
                  >
                    <input type="radio" name="confianca" value={n} required className="sr-only" />
                    {n}
                  </label>
                ))}
              </div>
              <p className="text-xs text-muted-foreground">1 = Insegurança total / 5 = Segurança plena</p>
            </div>

            <div className="space-y-2">
              <Label htmlFor="desafio">Maior desafio ou medo ao dirigir</Label>
              <Textarea id="desafio" placeholder="Conte-nos sobre seu maior desafio..." rows={3} />
            </div>
          </div>

          {/* Bloco 3 */}
          <h3 className="text-lg font-bold text-navy pt-4">Veículo & Disponibilidade</h3>
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="flex gap-4">
                {["Sim", "Não"].map((opt) => (
                  <label key={opt} className="flex items-center gap-2 text-sm">
                    <input type="radio" name="veiculo" value={opt} className="accent-electric" />
                    {opt}
                  </label>
                ))}
              </div>
            ...
            <div className="flex gap-3">
              {["Manhã", "Tarde", "Noite"].map((t) => (
                <label
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-2xl border border-border hover:border-electric cursor-pointer transition-colors text-sm"
                >
                  <input type="radio" name="turno" value={t} className="accent-electric" />
                  {t}
                </label>
              ))}
            </div>
